...
def get_weights(total_words_index, tfidf_text, tfidf_kw, dk, d_k, _dk, _d_k):
    ...
    for wid in total_words_index:
        if wid in dk:
            ...
            weights.append(0.9 + ((tkw[wid] - min_dk)/(max_dk))*1.1)
            #weights.append(4 + ((tkw[wid])/(max_dk))*3)
        elif wid in _dk:
            weights.append(0.5 + ((tkw[wid] - min__dk)/(max__dk))*0.7)
            #weights.append(1 + ((tkw[wid])/(max__dk))*1.5)
        elif wid in d_k:
            weights.append(0.2 + ((ttext[wid] - min_d_k)/(max_d_k))*0.4)
            #weights.append(0.7 + ((ttext[wid])/(max_d_k))*0.5)
        elif wid in _d_k:
            weights.append(0.01)
    return weights

def get_toptfidf(tfidf_text, tfidf_kw, dic):
    '''
    if no domain words, get top tfidf words as domain words
    '''
    ret = []
    id_ret = []
    overall = tfidf_text.items()
    sorted_overall = sorted(overall, key=lambda a: a[1], reverse=True)
    for item in sorted_overall[:3]:
        ret.append(dic[item[0]])
        id_ret.append(item[0])
    return ret, id_ret
...
def extract_kw(r, dic, tfidf_text, tfidf_kw, centroids, degree_diff_mode=None):
    ...
    toptfidf, toptfidf_index = get_toptfidf(tfidf_text, tfidf_kw, dic)
    if domain_words is None:
        #if no domain words, get top tfidf
        domain_words = toptfidf_index
    ...
    # if belongs to top words, add extra weights

    for i in range(len(total_words_index)):
        if total_words_index[i] in toptfidf_index:
            weights[i] += 0.4
        if total_words_index[i] in dk and not total_words_index[i] in toptfidf_index:
            weights[i] -= 0.3
