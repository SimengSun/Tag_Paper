...
def sequential_kmeans(data, k, k_inits,iter_num=None, init_a=None):
    ...
    flat_a = 0.00008  # flat learning rate
     # iter begin
    t = 0 # 0 < t < mn
    for m in range(iter_num):
        ...
        print("skm end iter {} at {} loss {}".format(m, datetime.datetime.now(), variance))
...
def kmeans(data, k, k_inits,iter_num=None):
    ...
    for m in range(iter_num):
        ...
        for di in range(len(data)):
            variance += euclidean_distances([centroids[tagged_data[di]]], [data[di]])[0][0]**2
        print("km end iter {} at {} loss {}".format(m, datetime.datetime.now(), variance))
...
def minibatchkmeans(data, k, k_inits, iter_num, batch_size):
    # assigned centroid index
    tagged_data = [0 for i in range(len(data))]
    data_last_dist = [-2 for i in range(len(data))]
    data_eu_last_dist = []
    ...
    for m in range(iter_num):
        ...
        for batch_count in range(batch_num):
            for i in range(batch_count*batch_size, (batch_count+1)*batch_size):
                last = data_last_dist[i]
                index = tagged_data[i]
                for ki in range(len(centroids)):
                    tmp = cosine_similarity([data[i]], [centroids[ki]])[0][0]
                    if tmp > last:
                        last = tmp
                        index = ki
                tagged_data[i] = index
                data_last_dist[i] = last
        for i in range(len(centroids)):
            this_set = [j for j in range(len(data)) if tagged_data[j] == i]
            sum_vec = 0
            for idx in this_set:
