...
import afk_mcmc_iter as afkmc2_iter
import datetime

def afkmc2_ap_test_iter_d(data,  chainlen, m, iter_num):
    points_list, index_list = afkmc2_iter.assumption_free_kmcmc_modified(data, m, chainlen, iter_num=iter_num)
    k_list = []
    labels_list = []
    final_index_list = []
    for i in range(iter_num):
        ap_model = AffinityPropagation()
        ap = ap_model.fit(points_list[i])
        k_list.append(len(ap.cluster_centers_indices_))
        labels_list.append(ap.labels_)
        final_index_list.append(index_list[i])

    #get most count
    item_set = set(k_list)
    count_dic = {}
    for item in item_set:
        count_dic[item] = k_list.count(item)
    sorted_count_dic = sorted(count_dic.items(), key=lambda a: a[1], reverse=True)
    return sorted_count_dic
...
def afkmc2_cluster_number_dbscan(data, tags,  chainlen, m):
    ...
    #visualize m_points
    low_dim_embs = tsne.fit_transform(m_points)
    plotvecs(low_dim_embs)
